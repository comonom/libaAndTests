    ...
    public class Calculations
    {
            ...
            int consultationTime)
        {
            if (startTimes.Length != durations.Length)
                throw new ArgumentException("Количество в startTimes[] не равно количеству duration[]");

            if (beginWorkingTime > endWorkingTime)
                throw new ArgumentException("Время начала рабочего дня больше чем конец рабочего дня");

            if(consultationTime <= 0)
                throw new ArgumentException("Время консультации не может быть меньше или равно 0");

            foreach(var span in startTimes)
            {
                if(span.Days > 0)
                    throw new ArgumentException("В timespan не может быть больше 0 дней.");
            }
